...
- **Projects** (`/projects`), **Games** (`/games`) — sections; Games hosts public playable builds, including SYL at `/games/syl/` and Fable Survival at `/games/fable-survival/`. SYL now includes the promoted Kimi expansion content, Heartbeat Realtime visibility MVP, and ship yaw/touch-turn fix; the unlisted `/games/syl-test/` lane remains available for future risky previews.
