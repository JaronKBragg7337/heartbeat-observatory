...
- **Projects** (`/projects`), **Games** (`/games`) — sections; Games hosts public playable builds, including stable SYL at `/games/syl/` and Fable Survival at `/games/fable-survival/`. The unlisted SYL test lane lives at `/games/syl-test/` for outside playtesting without replacing stable.
...
- **SYL + Fable Survival game lanes** — Fable Survival is hosted at `/games/fable-survival/` and appears on the Games page plus the in-world Games arcade. SYL stable remains untouched at `/games/syl/`; the unlisted `/games/syl-test/` lane carries the experimental Kimi expansion pack and Heartbeat realtime presence adapter so outside testers and AI browser agents can open it directly.
