...
- **Games** ([`/games`](https://www.heartbeatobservatory.com/games)) — public playable builds hosted inside Heartbeat Observatory. Games currently includes SYL, Fable Survival, and President Sim.

  - **SYL — Space You Land** ([`/games/syl/`](https://www.heartbeatobservatory.com/games/syl/)) — the permanent Three.js space game, delivered as one responsive full-fidelity phone and desktop-browser client. The live V1 currently proves continuous planetary traversal, ship repair, touch controls, and local saves while the canonical V2 rebuild moves toward server authority, volumetric planets, physical cargo, walkable interiors, and persistent component damage. Source and governing canon live in [`SYL-Full-Game`](https://github.com/JaronKBragg7337/SYL-Full-Game). The unrelated Fable Survival game remains separate. `/games/syl-test/` is not approved staging because it shares the production origin and legacy save key.

  - **Fable Survival** ([`/games/fable-survival/`](https://www.heartbeatobservatory.com/games/fable-survival/)) — a mobile-first survival prototype: scavenge, build, fight infected, survive the night, and see other Heartbeat players through the shared realtime presence layer. It is hosted inside Heartbeat Observatory so phone and browser playtests share the same public link and multiplayer shell.

  - **President Sim — The Executive Office** ([play](https://www.heartbeatobservatory.com/games/president-sim/)) — a 3D presidential decision game with walking visitors, adviser questions, desk briefings and consequences that return. Episode 01 connects AI infrastructure, a chip-export negotiation, a branching governor visit and a press interview. Includes four hearings, 81 policy paths, device-local saves, optional browser narration and a Cinema view. Dated sources distinguish the factual premises from fictional dialogue and simulation outcomes. The independent game is not affiliated with the White House. [Classic](https://www.heartbeatobservatory.com/games/president-sim/classic.html) preserves the earlier 16-round game. Canonical source and build instructions live in [President-Sim](https://github.com/JaronKBragg7337/President-Sim).
...
- **SYL + Fable Survival are separate games** — Fable Survival remains at `/games/fable-survival/`. SYL's canonical public entry is `/games/syl/`, backed by the separate `SYL-Full-Game` source repository. The old `/games/syl-test/` route is not isolated and must not be described as safe staging until it has a distinct save namespace, backend environment, and deployment contract.
