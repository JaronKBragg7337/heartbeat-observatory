...
- **Projects** (`/projects`) — the open-source project index for Heartbeat Observatory and community builds. It shows the Observatory itself, claimed plots, linked GitHub repos, and live repo details filled in automatically. People can claim empty plots inside the world, attach a repo, and have that project appear publicly with its source and status.

- **Games** (`/games`) — public playable builds hosted inside Heartbeat Observatory. Games currently includes SYL, Fable Survival, and President Sim.

  - **SYL — Space You Land** ([`/games/syl/`](https://www.heartbeatobservatory.com/games/syl/)) — a world-scale space game foundation: repair your ship piece by piece, launch, cross real space with no loading screens, craft expansion parts, and land on another world. SYL now includes the promoted Kimi expansion content, Heartbeat Realtime visibility, ship yaw/touch-turn fixes, and an opt-in phone-safe DEV editor via [`/games/syl/?dev=1`](https://www.heartbeatobservatory.com/games/syl/?dev=1); the unlisted `/games/syl-test/` lane remains available for future risky previews.
